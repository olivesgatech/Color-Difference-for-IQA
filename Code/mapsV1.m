  function [ssimMap, pcdmMap] =mapsV1(img1FullINT, img2FullINT,sc,T,D,alpha,Z)
...
for i=1:s1
   ...
   tempMat(i,j)=COL_DIST(squeeze(img1Full(i,j,:))',squeeze(img2Full(i,j,:))',T,D,alpha,Z);
    end    
end


% perColDistSmall=1./tempMat;
perColDistSmall=tempMat;

perColDist=imresize(perColDistSmall,[size(ssim_map,1),size(ssim_map,2)]);
